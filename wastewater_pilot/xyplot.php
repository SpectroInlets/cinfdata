...
$query = "SELECT distinct " . $settings["grouping_column"] . ", comment, measurement_name FROM " .  $settings["measurements_table"] . " where type = " . $settings["type"] . " order by time desc limit 25000";

foreach ($db->query($query) as $row){
    $datelist[] = $row[0];
    $commentlist[] = $row[1];
    $measurement_name_list[] = $row[2];
}
             ...
             <select id="measurement_select" name="chosen_group[]" multiple size="8" onChange="showData(this.value, <?php echo("'" . $type . "'"); ?>)">
              <?php
                ...
                for($i=0;$i<count($datelist);$i++){
                    ...
                    echo("<option value=\"" . $datelist[$i] . "\" " . $selected . ">" . $datelist[$i] . ":&nbsp;&nbsp;&nbsp;" . $measurement_name_list[$i] . " --> " . $commentlist[$i] . "</option>\n");
                }
              ?>
             </select>
           </div>
